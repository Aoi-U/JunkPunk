...
void CAudioEngine::GetEventParameter(const string& strEventName, const string& strParameterName, float* parameter) {
	...
	if (tFoundIt == sgpImplementation->mEvents.end())
		return;
	float value = 0.0f;
	FMOD_RESULT result = tFoundIt->second->getParameterByName(strParameterName.c_str(), &value);
	CAudioEngine::ErrorCheck(result);
	if (parameter) {
		*parameter = value;
	}
}
...
void CAudioEngine::SetEventParameter(const string& strEventName, const string& strParameterName, float fValue) {
	...
	if (tFoundIt == sgpImplementation->mEvents.end())
		return;
	FMOD_RESULT result = tFoundIt->second->setParameterByName(strParameterName.c_str(), fValue);
	CAudioEngine::ErrorCheck(result);
}
